...
public class CapacitorCommunityUrlRequest: NSObject, URLSessionTaskDelegate {
    private var request: URLRequest;
    private var headers: [String:String];
    
    enum CapacitorCommunityUrlRequestError: Error {
    ...
    private func getRequestDataAsJson(_ data: JSValue) throws -> Data? {
        ...
        if JSONSerialization.isValidJSONObject(data) {
            ...
            throw CapacitorCommunityUrlRequest.CapacitorCommunityUrlRequestError.serializationError("[ data ] argument for request of content-type [ application/json ] must be serializable to JSON")
        }
    }
    ...
    private func getRequestDataAsFormUrlEncoded(_ data: JSValue) throws -> Data? {
        ...
        guard let obj = data as? JSObject else {
            ...
            throw CapacitorCommunityUrlRequestError.serializationError("[ data ] argument for request with content-type [ multipart/form-data ] may only be a plain javascript object")
        }
    ...
    private func getRequestDataAsMultipartFormData(_ data: JSValue) throws -> Data {
        guard let obj = data as? JSObject else {
            // Throw, other data types explicitly not supported.
            throw CapacitorCommunityUrlRequestError.serializationError("[ data ] argument for request with content-type [ application/x-www-form-urlencoded ] may only be a plain javascript object")
        }
    ...
    private func getRequestDataAsString(_ data: JSValue) throws -> Data {
        guard let stringData = data as? String else {
            throw CapacitorCommunityUrlRequestError.serializationError("[ data ] argument could not be parsed as string")
        }
    ...
    func getRequestData(_ body: JSValue, _ contentType: String) throws -> Data? {
        ...
        if let strVal = try? getRequestDataAsString(body) {
            ...
            throw CapacitorCommunityUrlRequestError.serializationError("[ data ] argument could not be parsed for content type [ \(contentType) ]")
        }
    }
