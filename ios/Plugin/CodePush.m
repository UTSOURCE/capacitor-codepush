#if __has_include(<Capacitor/Capacitor-Swift.h>)
...
// suppressing warnings of the type: "Class 'CodePushPlugin' does not conform to
// protocol 'CAPBridgedPlugin'" protocol conformance for this class is
// implemented by a macro and clang isn't detecting that
...
- (void)getBinaryHash:(CAPPluginCall *)call {
  ...
  if (binaryHash) {
    ...
    if (error) {
      [call reject:
                @"An error occurred when trying to get the hash of the binary "
                 @"contents. ":nil:error:@{}];
    } else {
...
- (void)getPackageHash:(CAPPluginCall *)call {
  ...
  if (!path) {
    ...
    if (error) {
      [call reject:[NSString stringWithFormat:@"An error occured when trying "
                                              @"to get the hash of %@. %@",
...
- (void)getPublicKey:(CAPPluginCall *)call {
  ...
  if (!publicKey) {
    [call resolve:nil]; /* Returning @{@"value":nil} causes an uncaught
                           exception */
  } else {
...
- (void)decodeSignature:(CAPPluginCall *)call {
  NSString *publicKey = [self getString:call
                                  field:@"publicKey"
                           defaultValue:nil];

  // remove BEGIN / END tags and line breaks from public key string
  publicKey = [publicKey
      stringByReplacingOccurrencesOfString:@"-----BEGIN PUBLIC KEY-----\n"
                                withString:@""];
  publicKey = [publicKey
      stringByReplacingOccurrencesOfString:@"-----END PUBLIC KEY-----"
                                withString:@""];
  ...
  id<JWTAlgorithmDataHolderProtocol> verifyDataHolder =
      [JWTAlgorithmRSFamilyDataHolder new]
          .keyExtractorType([JWTCryptoKeyExtractor publicKeyWithPEMBase64].type)
          .algorithmName(@"RS256")
          .secret(publicKey);
  JWTCodingBuilder *verifyBuilder =
      [JWTDecodingBuilder decodeMessage:jwt].addHolder(verifyDataHolder);
  JWTCodingResultType *verifyResult = verifyBuilder.result;
  if (verifyResult.successResult) {
    NSLog(@"JWT signature verification succeeded, payload content:  %@",
          verifyResult.successResult.payload);
    [call resolve:@{
      @"value" : verifyResult.successResult.payload[@"contentHash"]
    }];
  } else {
    [call reject:[@"Signature verification failed: "
                     stringByAppendingString:verifyResult.errorResult.error
                                                 .description
    ]:nil:nil:@{}];
  }
}
...
- (void)handleUnconfirmedInstall:(BOOL)navigate {
  if ([CodePushPackageManager installNeedsConfirmation]) {
    ...
    CodePushPackageMetadata *currentMetadata =
        [CodePushPackageManager getCurrentPackageMetadata];
    rollbackStatusReport =
        [[StatusReport alloc] initWithStatus:UPDATE_ROLLED_BACK
    ...
    if (navigate) {
      CodePushPackageMetadata *currentMetadata =
          [CodePushPackageManager getCurrentPackageMetadata];
      bool revertSuccess = (nil != currentMetadata &&
                            [self loadPackage:currentMetadata.localPath]);
      if (!revertSuccess) {
...
- (void)notifyApplicationReady:(CAPPluginCall *)call {
  if ([CodePushPackageManager isBinaryFirstRun]) {
    ...
    StatusReport *statusReport =
        [[StatusReport alloc] initWithStatus:STORE_VERSION
                                    andLabel:nil
    ...
    CodePushPackageMetadata *currentMetadata =
        [CodePushPackageManager getCurrentPackageMetadata];
    StatusReport *statusReport =
        [[StatusReport alloc] initWithStatus:UPDATE_CONFIRMED
                                    andLabel:currentMetadata.label
                               andAppVersion:currentMetadata.appVersion
                            andDeploymentKey:currentMetadata.deploymentKey];
    [CodePushReportingManager reportStatus:statusReport
    ...
    [CodePushReportingManager
        reportStatus:[CodePushReportingManager getAndClearFailedReport]
         withWebView:self.webView];
  }
...
- (NSNumber *)getNumber:(CAPPluginCall *)call
...
- (void)install:(CAPPluginCall *)call {
  NSString *location = [self getString:call
                                 field:@"startLocation"
                          defaultValue:nil];
  NSNumber *installMode = [self getNumber:call
                                    field:@"installMode"
                             defaultValue:IMMEDIATE];
  NSNumber *minimumBackgroundDuration =
      [self getNumber:call field:@"minimumBackgroundDuration" defaultValue:0];
...
- (void)reportFailed:(CAPPluginCall *)call {
  ...
  if (statusReportDict) {
    [CodePushReportingManager
        saveFailedReport:[[StatusReport alloc]
                             initWithDictionary:statusReportDict]];
  }
  [call resolve];
}
...
- (void)reportSucceeded:(CAPPluginCall *)call {
  ...
  if (statusReportDict) {
    [CodePushReportingManager
        saveSuccessfulReport:[[StatusReport alloc]
                                 initWithDictionary:statusReportDict]];
  }
  [call resolve];
}
...
- (void)restartApplication:(CAPPluginCall *)call {
  ...
  CodePushPackageMetadata *deployedPackageMetadata =
      [CodePushPackageManager getCurrentPackageMetadata];
  if (deployedPackageMetadata && deployedPackageMetadata.localPath &&
      [self getStartPageURLForLocalPackage:deployedPackageMetadata.localPath]) {
...
- (void)sendResultForPreference:(NSString *)preferenceName
  ...
  if ([preferenceValue length] > 0) {
    ...
    [call reject:[NSString stringWithFormat:@"Could not find preference %@",
                                            preferenceName
    ]:nil:nil:@{}];
  }
}
...
- (void)clearDeploymentsIfBinaryUpdated {
  ...
  if (deployedPackageMetadata) {
    NSString *deployedPackageNativeBuildTime =
        deployedPackageMetadata.nativeBuildTime;
    ...
    if (deployedPackageNativeBuildTime != nil && applicationBuildTime != nil &&
      ...
      if (![deployedPackageNativeBuildTime
              isEqualToString:applicationBuildTime] ||
...
- (void)load {
  ...
  // handle both ON_NEXT_RESUME and ON_NEXT_RESTART - the application might have
  // been killed after transitioning to the background
  if (pendingInstall && (pendingInstall.installMode == ON_NEXT_RESTART ||
                         pendingInstall.installMode == ON_NEXT_RESUME)) {
    [self markUpdate];
    [CodePushPackageManager clearPendingInstall];
  }
}
...
- (void)applicationWillEnterForeground {
  ...
  long durationInBackground =
      lastResignedDate ? [[NSDate date] timeIntervalSinceDate:lastResignedDate]
                       : 0;
  if (pendingInstall && pendingInstall.installMode == ON_NEXT_RESUME &&
      durationInBackground >= pendingInstall.minimumBackgroundDuration) {
    CodePushPackageMetadata *deployedPackageMetadata =
        [CodePushPackageManager getCurrentPackageMetadata];
    if (deployedPackageMetadata && deployedPackageMetadata.localPath) {
...
- (void)applicationWillResignActive {
  // Save the current time so that when the app is later resumed, we can detect
...
- (void)setServerBasePath:(NSString *)serverPath {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    [(CAPBridgeViewController *)self.bridge.viewController
        setServerBasePathWithPath:serverBasePath];
  });
}
...
- (NSString *)getConfigLaunchUrl {
  ...
  //    ofType:@"xml"]; NSURL* configUrl = [NSURL fileURLWithPath:configPath];
  //
  //    NSXMLParser* configParser = [[NSXMLParser alloc]
  //    initWithContentsOfURL:configUrl]; [configParser setDelegate:((id <
  //    NSXMLParserDelegate >)delegate)]; [configParser parse];
  // TODO: implement me
...
- (NSURL *)getStartPageURLForLocalPackage:(NSString *)packageLocation {
  if (packageLocation) {
    NSString *libraryLocation = [NSSearchPathForDirectoriesInDomains(
        NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
    NSArray *realLocationArray =
        @[ libraryLocation, packageLocation, @"public/index.html" ];
    NSString *realStartPageLocation =
        [NSString pathWithComponents:realLocationArray];
    if ([[NSFileManager defaultManager]
            fileExistsAtPath:realStartPageLocation]) {
      // Fixes WKWebView unable to load start page from CodePush update
      // directory
      NSString *scheme = [self getAppScheme];
      if ([Utilities CDVWebViewEngineAvailable] &&
          ([realStartPageLocation hasPrefix:@"/_app_file_"] == NO) &&
          !([scheme isEqualToString:@"file"] || scheme == nil)) {
        realStartPageLocation =
            [@"/_app_file_" stringByAppendingString:realStartPageLocation];
      }
...
- (void)isFailedUpdate:(CAPPluginCall *)call {
  NSString *packageHash = [self getString:call
                                    field:@"packageHash"
                             defaultValue:nil];
  if (nil == packageHash) {
...
- (void)isFirstRun:(CAPPluginCall *)call {
  ...
  NSString *packageHash = [self getString:call
                                    field:@"packageHash"
                             defaultValue:nil];
  CodePushPackageMetadata *currentPackageMetadata =
      [CodePushPackageManager getCurrentPackageMetadata];
  if (currentPackageMetadata) {
    isFirstRun =
        (nil != packageHash && [packageHash length] > 0 &&
...
- (void)unzip:(CAPPluginCall *)call {
  ...
  NSString *unzipPath = [self getString:call
